...
Ship Core Framework is a Space Engineers mod centered on core-driven ship rules. It lets a grid group inherit limits, modifiers, speed behavior, defense tuning, placement rules, and upgrade-module effects from a selected ship core or no-core profile.

The mod package lives in [`ShipCoreFramework/`](../ShipCoreFramework/). It requires a content pack that supplies core definitions and a no-core profile, such as [Arcane Ship Cores](arcane-ship-cores.md).

Developer guidance for the runtime client/server boundary is in
[the authority layout guide](authority-layout.md).
...
Then open `http://localhost:8080/` (it redirects to `scf-configurator/`).
